...
function runCalculadora2() {
    ...
    const noPrimosSpan = document.getElementById('noPrimosRes');
    const mensajes = document.getElementById('mensajes');

    if (!input || !multSpan || !primosSpan || !noPrimosSpan || !mensajes) return;

    mensajes.textContent = '';
    multSpan.textContent = '';
    primosSpan.textContent = '';
    if (noPrimosSpan) noPrimosSpan.textContent = ''; 
    ...
    if (nInt > 100000) {
        ...
        if (noPrimosSpan) {
            const primosSet = new Set(primos);
            const noPrimos = [];
            for (let i = 1; i <= nInt; i++) {
                if (!primosSet.has(i)) noPrimos.push(i);
            }
            noPrimosSpan.textContent = noPrimos.join(', ') || 'No hay números no primos en este rango.';
        }
    }
}
